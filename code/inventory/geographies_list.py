    ...
    for dir in os.listdir(dirpath):
        ...
        for path in Path(subdir).rglob("data/distribution/**/*"):
            ...
            if path.suffix in [".geojson"]:                
                ...
                try:
                    # KRISTIAN - change this line to read in a geojson
                    path = str(path)
                    df = gpd.read_file(path, ignore_geometry=True)
                    
                    # KRISTIAN FILL IN CODE HERE - remove geographies column of df
                    df = pd.DataFrame(df)
...
if __name__ == "__main__":
    ...
    # write inventory    
    inv_final_df.to_csv("./docs/geographies.csv", index=False)
