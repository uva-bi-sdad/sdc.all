# tests for valid measure_types in data files 
            ...
            for f in path.iterdir(): #os.listdir(path):
                ...
                if f.suffix in [".xz", ".csv"]:
                    ...
                    try:
                        ...
                        data_measure_types = df["measure_type"].unique()   
                         
                        nonvalid_types = list(set(data_measure_types).difference(set(valid_types)))    
                            
                        if len(nonvalid_types) == 0:
                            report += "\t<p><font color='#009E73'> [VALID] </font> %s: %s </p>\n" % (parent_dir, f.name) 
                        else: 
                            report += "\t<p><font color='#D55E00'> [INVALID MEASURE TYPES] </font> %s %s: %s </p>\n" % (nonvalid_types, parent_dir, f.name)
                        
                    except:
                        print(traceback.format_exc())
                        report += "\t<p><font color='#D55E00'> [ERROR - NO MEASURE TYPE COLUMN] </font> %s: %s</p>\n" % (parent_dir, f.name)
...
if __name__ == "__main__":
    ...
    test = Test(
        __file__,
        "Measure Types Validity Test",
        "Checks whether measure types in data files are valid",
    )
