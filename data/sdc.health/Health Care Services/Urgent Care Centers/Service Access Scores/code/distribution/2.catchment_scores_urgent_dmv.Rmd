...
#working directory
```{r}
setwd("~/VDH/Floating_Catchment_Areas/dmv/7.1 urgent_dmv_model_2022")
```
# read data
```{r}
...
pal <- colorBin("RdYlBu", population$providers_2sfca, bins = 8)

plot_2sfca <- map %>%
  addControl("Urgent Care Centers Per 10,000 People (2-Step Floating Catchment Area), 2022", "topright") %>%
...
pal <- colorBin("RdYlBu", population$providers_e2sfca, bins = 8)

plot_e2sfca <- map %>%
  addControl("Urgent Care Centers  Per 10,000 People (enhanced 2-Step Floating Catchment Area), 2022", "topright") %>%
...
pal <- colorBin("RdYlBu", population$providers_3sfca, bins = 8)

plot_3sfca <- map %>%
  addControl("Urgent Care Centers Per 10,000 People (3-Step Floating Catchment Area), 2022", "topright") %>%
...
dmv_bg_2021_acccess_scores_hospitals <- dmv_bg_2021_acccess_scores_hospitals %>%  
  mutate(measure = case_when(measure == "hosp_2sfca" ~ "urgent_2sfca",
                             measure == "hosp_3sfca" ~ "urgent_3sfca",
                             measure == "hosp_cnt" ~ "urgent_cnt",
                             measure == "hosp_e2sfca" ~ "urgent_e2sfca",
                             measure == "hosp_near_10_mean" ~ "urgent_near_10_mean",
                             measure == "hosp_near_10_median" ~ "urgent_near_10_median",
                             measure == "hosp_pop_cnt" ~ "urgent_pop_cnt")
)
...
# write.csv(dmv_bg_2021_acccess_scores_hospitals, "ncr_bgtrct_gmap_2022_access_scores_urgent_care_centers.csv", row.names = FALSE )
readr::write_csv(dmv_bg_2021_acccess_scores_hospitals, xzfile("ncr_bgtrct_gmap_2022_access_scores_hospitals.cxv.xz", compression = 9))
