...
provider_ncr <- read_csv("~/test_symlink/git/sdc.health_dev/Health Care Services/Urgent Care Centers/Service Access Scores/data/working/ncr_pt_gmap_2022_urgent_care_centers_10k.csv")
#limit the extrem values
 provider_ncr_lim <- provider_ncr %>% filter( lat < 40.28503 & lat > 37.89674 ) %>% filter( long < -76.09205 & long > -78.48800 )
...
# save new data
```{r}
write.csv(provider[, c("ID", "name", "lat", "lon")], "provider.csv", row.names = FALSE)
