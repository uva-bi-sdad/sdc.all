...
# Working directory
```{r}
setwd("~/VDH/Floating_Catchment_Areas/va/7. urgent_care_va_model")
```
# Read data
```{r}
...
plot_2sfca <- map %>%
  addControl("Urgent care Per 10,000 People (2-Step Floating Catchment Area)", "topright") %>%
...
plot_e2sfca <- map %>%
  addControl("Urgent care Per 10,000 People (enhanced 2-Step Floating Catchment Area)", "topright") %>%
...
plot_3sfca <- map %>%
  addControl("Urgent care Per 10,000 People (3-Step Floating Catchment Area)", "topright") %>%
...
write.csv(va_bg_2021_access_scores_urgent, "va_hdcttrbg_hifld_2021_access_scores_urgent.csv", row.names = FALSE )
