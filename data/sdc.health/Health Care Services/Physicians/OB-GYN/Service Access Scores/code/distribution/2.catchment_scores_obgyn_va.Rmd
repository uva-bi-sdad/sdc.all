...
traveltimes <- read.csv("traveltimes_trimmed_20.csv", row.names = "GEOID")
...
map <- leaflet(blockgroup_shapes, options = leafletOptions(attributionControl = FALSE)) %>%
  ...
  addCircles(
    ...
    label = ~ paste0("ID: ", ID, "  Number Doctors:", doctors), group = "Urgent", options = pathOptions(pane = "points")
  ) %>%
...
plot_2sfca <- map %>%
  ...
  addPolygons(
    ...
    label = paste0(
      "GEOID: ", population_plot$GEOID, ", Population Obgyn: ", population_plot$obgyn_pop,
...
plot_e2sfca <- map %>%
  ...
  addPolygons(
    ...
    highlightOptions = highlightOptions(color = "#fff"), group = "Access",
    label = paste0(
      "GEOID: ", population_plot$GEOID, ", Population Obgyn: ", population_plot$population,
...
densities_fca <- ggplot(data=CombinedData, aes(x=Observation, group=fca, fill=fca)) +
  ...
  labs(title = 'Distribution of Floating Catchment Areas - Virginia - ObGyn')
...
```{r}
# Reshape wide-long
#DEFINE relevant population and names: population_plot is the relevant file for VA only
...
# write.csv(va_bg_2021_access_scores_obgyn, "va_hdcttrbg_webmd_2021_access_scores_obgyn.csv", row.names = FALSE )
readr::write_csv(va_bg_2021_access_scores_obgyn, xzfile("va_hdcttrbg_webmd_2021_access_scores_obgyn.csv.xz", compression = 9))
