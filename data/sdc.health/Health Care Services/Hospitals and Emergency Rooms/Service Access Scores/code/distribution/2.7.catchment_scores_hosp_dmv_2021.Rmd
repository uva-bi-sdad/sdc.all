...
#working directory
```{r}
setwd("~/VDH/Floating_Catchment_Areas/dmv/5.1 hospitals_dmv_model_2015-2022/5.1 hospitals_dmv_model_2021")
```
# read data
```{r}
...
blockgroup_shapes_va <- st_transform( tigris::block_groups(state = c("VA"), class = "sf", year=2020), 4326)
blockgroup_shapes_dc <- st_transform( tigris::block_groups(state = c("DC"), class = "sf", year=2020), 4326)
blockgroup_shapes_md <- st_transform( tigris::block_groups(state = c("MD"), class = "sf", year=2020), 4326)
blockgroup_shapes_pa <- st_transform( tigris::block_groups(state = c("PA"), class = "sf", year=2020), 4326)
blockgroup_shapes_wv <- st_transform( tigris::block_groups(state = c("WV"), class = "sf", year=2020), 4326)
...
##new
shape_counties <- tigris::counties(state = c("VA", "MD", "DC"), class = "sf", year=2020)
...
plot_2sfca <- map %>%
  addControl("Hospitals Per 100,000 People (2-Step Floating Catchment Area) in 2021", "topright") %>%
...
plot_e2sfca <- map %>%
  addControl("Hospitals Per 100,000 People (enhanced 2-Step Floating Catchment Area) in 2021", "topright") %>%
...
plot_3sfca <- map %>%
  addControl("Hospitals Per 100,000 People (3-Step Floating Catchment Area) in 2021", "topright") %>%
...
# write.csv(dmv_bg_2021_acccess_scores_hospitals, "ncr_bgtrct_2021_acccess_scores_hospitals", row.names = FALSE )
readr::write_csv(dmv_bg_2021_acccess_scores_hospitals, xzfile("ncr_bgtrct_2021_access_scores_hospitals.csv.xz", compression = 9))
