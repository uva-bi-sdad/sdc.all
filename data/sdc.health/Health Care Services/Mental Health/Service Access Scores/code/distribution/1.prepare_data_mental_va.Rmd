---
title: "catchment_scores - mental health - VA"
author: "--"
date: "2/4/2022"
...
#   query= "
# SELECT  *
# FROM dc_health_behavior_diet.va_pl_samhsa_2021_mental_health_facilities " 
# )
#WHERE state= 'VA' 

# provider <- provider %>% select( name1, street1, city, state, county, zip, lat=latitude, lon=longitude)
...
provider <- provider_us %>% filter(type_facility == 'MH'  ) %>% filter( state == 'VA' | fips %in% buffer_counties_state$identifier)
names(provider)[names(provider)=='latitude'] <- 'lat'
names(provider)[names(provider)=='longitude'] <- 'lon'


#Save list of mental health facilities in VA
#write_csv(provider, "va_hifld_2021_mental_health.csv")
