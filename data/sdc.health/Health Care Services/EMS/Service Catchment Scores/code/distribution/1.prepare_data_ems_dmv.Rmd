...
provider <- sf::st_read(
  ...
  query= "
SELECT  *
FROM dc_health_behavior_diet.ncr_pl_hifld_2021_ems_stations  "
)

provider <- provider %>% dplyr::select(unique_ID, facility_name, address, city, state, county, fips, lat, lon)

#c) Disconnect
RPostgreSQL::dbDisconnect(con)

#write
write.csv(provider, "ems_stations.csv" )
```
# load data provider
```{r}
...
#summary by geoid: counts
provider_counts <- provider %>% group_by(GEOID) %>% summarise( hosp_cnt= sum(doctors))

#join hosp_cnt to geographies
data_combined <- data_combined %>% left_join(provider_counts, by= c("GEOID") )
```
...
population <- data_combined %>% dplyr::select(GEOID, population, hosp_cnt, near_10_mean, near_10_median)
