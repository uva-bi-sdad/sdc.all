...
average_years_schooling_2017 = convert_tract_to_2020(education_raw_2017[!duplicated(education_raw_2017$geoid, fromLast = TRUE),], 2017)

# for access to food we need to use 2017 data for access to food, does not exist for 2017
...
## Combine 2017 Predictors

```{r}
list_2017 = list(access_care_2017, average_years_schooling_2017, employment_access_2017, labor_force_2017, population_density_2017, walkability_2021, segregation_2017, income_inequality_2017, affordability_2017, environment_hazard_2022, access_to_food_new_2017, material_deprivation_indicator_2017, incarceration %>% filter(year == 2017), mobility_2017)
...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`Accees to Care`, `LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2017$Incarceration))
...
inc_2017_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2017 %>% dplyr::select(-c(geoid, `Accees to Care`)))

cor(inc_2017_predicted_normalized, data_matrix$Incarceration)
```


## Aggregate data to predict 2017 HOI

```{r}
indicators_2017 = hoi_recreated_normalized_2017 %>% mutate(Incarceration = inc_2017_predicted_normalized)
```
...
HOI_data_2017_quintile = HOI_data_2017 %>% mutate(value= as.numeric(cut(HOI_data_2017$value, breaks = 5, labels = FALSE)))

#write.csv(HOI_data_2017_quintile, "health_opportunity_indicator_2017_quintiles.csv")
```

```{r}
incarceration_data_2017 = indicators_2017 %>% select(geoid) %>% mutate(year = 2017) %>% mutate(measure = "incarceration_rate_per_100000") %>% mutate(value = round(pmax(inc_2017_predicted, 0),0)) %>% mutate(moe = NA) # add a relu type function to get rid of negative values and round

setwd("incarceration_raw_predicted")
#write.csv(incarceration_data_2017, "incarceration_rate_per_100000_2017.csv")
...
setwd("hoi_predictors/2017_normalized")
...
i = 1
 for (column in colnames(indicators_2017 %>% dplyr::select(-c(geoid)))) {
   data_to_save_normalized = indicators_2017 %>% select(geoid, all_of(column)) %>% mutate(year = 2017) %>% mutate(measure = measure_labels[i]) %>% rename(value = column) %>% mutate(moe = NA) 
   
   print(data_to_save_normalized)
   
   #write.csv(data_to_save_normalized, glue('{measure_labels[i]}_normalized_2017.csv'))
...
i = 1
for (column in colnames(hoi_recreated_imputed_2017 %>% dplyr::select(-c(geoid, Incarceration)))) {
   data_to_save_raw = hoi_recreated_imputed_2017 %>% select(geoid, all_of(column)) %>% mutate(year = 2017) %>% mutate(measure = measure_labels_raw[i]) %>% rename(value = column) %>% mutate(moe = NA) 
   
   print(data_to_save_raw)
   
   #write.csv(data_to_save_raw, glue('{measure_labels[i]}_2017.csv'))
