...
# add this to health_dev repo with code files
```
...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`Accees to Care`, `LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2019$Incarceration))
...
inc_2019_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2019 %>% dplyr::select(-c(geoid, `Accees to Care`)))

cor(inc_2019_predicted_normalized, data_matrix$Incarceration)
```
...
```{r}
indicators_2019 = hoi_recreated_normalized_2019 %>% mutate(Incarceration = inc_2019_predicted_normalized)
```
...
HOI_data_2019_quintile = HOI_data_2019 %>% mutate(value = as.numeric(cut(HOI_data_2019$value, breaks = 5, labels = FALSE)))

#write.csv(HOI_data_2019_quintile, "health_opportunity_indicator_2019_quintiles.csv")
```

```{r}
incarceration_data_2019 = indicators_2019 %>% select(geoid) %>% mutate(year = 2019) %>% mutate(measure = "incarceration_rate_per_100000") %>% mutate(value = round(pmax(inc_2019_predicted, 0),0)) %>% mutate(moe = NA) # add a relu type function to get rid of negative values and round
...
setwd("hoi_predictors/2019_normalized")
...
i = 1
 for (column in colnames(indicators_2019 %>% dplyr::select(-c(geoid)))) {
   data_to_save_normalized = indicators_2019 %>% select(geoid, all_of(column)) %>% mutate(year = 2019) %>% mutate(measure = measure_labels[i]) %>% rename(value = column) %>% mutate(moe = NA) 
   
   print(data_to_save_normalized)
   
   #write.csv(data_to_save_normalized, glue('{measure_labels[i]}_normalized_2019.csv'))
...
i = 1
for (column in colnames(hoi_recreated_imputed_2019 %>% dplyr::select(-c(geoid, Incarceration)))) {
   data_to_save_raw = hoi_recreated_imputed_2019 %>% select(geoid, all_of(column)) %>% mutate(year = 2019) %>% mutate(measure = measure_labels_raw[i]) %>% rename(value = column) %>% mutate(moe = NA) 
   
   print(data_to_save_raw)
   
   #write.csv(data_to_save_raw, glue('{measure_labels[i]}_2019.csv'))
