...
orig_df <- read_excel("data/original/pop_churn.xlsx")
...
out_df <- df_tracts_nodups %>% 
  rename("geoid"= "Ctfips",
        "pop_churning_quintile" = "quintiles",
        "pop_churning_level" = "Indicator Selector")
out_df <- out_df[,c("geoid", "pop_churning_quintile", "pop_churning_level")]
...
# save the dataset 
write_csv(out_df, "data/distribution/va_tr_vdh_2017_pop_churning_index.csv")
