...
orig_df <- read_excel("Population Health/Health Opportunity Index/data/original/emp_access.xlsx")
...
out_df <- df_tracts_nodups %>% 
  rename( "geoid"= "Ctfips",
          "employment_access_indicator" = "quintiles")
out_df <- out_df[,c("geoid", "employment_access_indicator")]
...
# county$value <- county$ct_val / county$ct_pop
...
# hlth_dis$value <- hlth_dis$hd_val / hlth_dis$hd_pop
...
# save the dataset 
write_csv(hdcttr, xzfile(paste0("Population Health/Health Opportunity Index/data/distribution/va_hdcttr_vdh_2017_employment_accessibility_index.csv.xz"), compression = 9) )


# save the dataset 
# write_csv(out_long, xzfile("Population Health/Health Opportunity Index/data/working/tract_data/va_tr_vdh_2017_employment_accessibility_index.csv.xz", compression = 9))
