...
cor(inc_2015_predicted, data_matrix$Incarceration)
```
...
```{r}
indicators_2015 = hoi_recreated_normalized_2015 %>% mutate(Incarceration = inc_2015_predicted)
```
...
```{r}
incarceration_data_2015 = indicators_2015 %>% select(geoid) %>% mutate(year = 2015) %>% mutate(measure = "incarceration_rate_per_100000") %>% mutate(value = inc_2015_predicted) %>% mutate(moe = NA)

incarceration_data_2015

#write.csv(incarceration_data_2015, "incarceration_rate_2015")
```
