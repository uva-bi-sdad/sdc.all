# Replicating Population Density Indicator of the VHD's HOI Community Environmental Profile
# A measure of population density that takes into account the density levels most people in the community experience.
...
orig_df <- read_excel("Population Health/Health Opportunity Index/data/original/pop_density.xlsx")
...
out_df <- df_tracts_nodups %>% 
  rename("geoid"= "Ctfips",
        "pop_density_indicator" = "quintiles")
out_df <- out_df[,c("geoid", "pop_density_indicator")]
...
# save the dataset 
write_csv(out_long, xzfile("Population Health/Health Opportunity Index/data/working/tract_data/va_tr_vdh_2017_pop_density_index.csv.xz", compression = 9))
