...
st_prev_logit <- qlogis(unname(svymean(~menthlth_binary, brfss_2016_design)[1]))
...
# mental health #
data_list_ment <- list(N = nrow(va_brfss),
                       y = va_brfss$menthlth_binary,
                       J = ncol(ps_mat),
...
brfss_mrp_fit_ment <- brfss_mrp$sample(data = data_list_ment,
                                       seed = 50,
                                       refresh = 0,
                                       parallel_chains = 10,
                                       iter_warmup = 2500,
...
saveRDS(ment_tract_res, "/project/biocomplexity/sdad/projects_data/vdh/dc_health_behavior_diet/brfss/ment_2016_pred.rds")
