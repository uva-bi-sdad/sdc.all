...
page_menu(
  input_checkbox(
    "Starting Layer", c("county", "tract"), 0, c("Counties", "Census Tracts"),
  ...
  page_section(
    ...
    input_combobox(
      ...
      id = "region_select", clearable = TRUE
    ),
...
input_variable("shapes", list(
  "region_select && shape_type == county" = "tract"
), "shape_type", list(
  county = "Counties",
  tract = "Census Tracts"
))
...
page_section(
  ...
  output_text(c(
    "(Entire Region)[r region_select]",
    "? > {region_select}"
  ), class = "compact"),
  output_text(list(
    "default" = "{shapes}",
    "region_select" = "{region_select} {shapes}"
