...
ncr_tract_emprt_all <- ncr_tract_emprt_all[, c("geoid", "year", "measure", "value", "moe")]
...
ncr_county_emprt_all <- ncr_county_emprt_all[, c("geoid", "year", "measure", "value", "moe")]

# Combine tract and counties
ncr_cttr_employment_rate <-
  rbindlist(list(ncr_tract_emprt_all, ncr_county_emprt_all))

# Write file
readr::write_csv(ncr_cttr_employment_rate, xzfile(paste0("Employment/data/distribution/ncr_cttr_", min(yrs), "_", max(yrs), "_employment_rate.csv.xz"), compression = 9))
