...
townsend_data <- townsend_data %>%
  group_by(region_type, year) %>%
  mutate(
    mean_Townsend = mean(Townsend_Index_z_sum, na.rm = TRUE),
    sd_Townsend = sd(Townsend_Index_z_sum, na.rm = TRUE),
    Townsend_Index_z_standardized = (Townsend_Index_z_sum - mean_Townsend) / sd_Townsend
  ) %>%
  ungroup() %>%
  select(-mean_Townsend, -sd_Townsend)
