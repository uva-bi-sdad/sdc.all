...
#upload data from fairfax (demography and geometry). filter on age
...
fairfax_pc_dmg <- fairfax_pc_dmg %>% select(geoid,year,measure,value) %>% filter(measure %in% c('pop_under_20','pop_20_64','pop_65_plus','total_pop'))
...
readr::write_csv(fairfax_newgeo_dmg, xzfile(paste0(savepath,"va059_hsrpdsdzc_sdad_20092020_age_demographics.csv.xz"), compression = 9))
...
arl_pc_dmg <- arl_pc_dmg %>% select(geoid,year,measure,value) %>% filter(measure %in% c('pop_under_20','pop_20_64','pop_65_plus','total_pop'))
...
# save the data ----------------------------------------------------------------------------------
savepath = "Age/data/distribution/"
readr::write_csv(arl_newgeo_dmg, xzfile(paste0(savepath,"va013_civic_sdad_20092019_age_demographics.csv.xz"), compression = 9))
