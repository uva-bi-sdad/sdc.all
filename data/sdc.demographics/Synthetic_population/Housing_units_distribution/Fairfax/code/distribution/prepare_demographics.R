...
fairfax_parcel_dmg <- merge(temp_acs2, temp, by.x='geoid', by.y='bg_geo', all.y=T) %>%
...
for (year in years) {
  readr::write_csv(fairfax_parcel_dmg[fairfax_parcel_dmg$year==year,], xzfile(paste0(savepath,"va059_pc_sdad_",year,"_demographics.csv.xz"), compression = 9))
}
