...
# upload the data ---------------------------------------------------------------------------------------------
uploadpath = "population/VA/fairfax/overall_fairfax/new_geography_synthetic/housing_units_distribution_method/parcels/data/"
...
parcels_dmgs_sd_geo_sf <- parcels_dmgs_sd_geo %>%
...
parcels_dmgs_sd_geo <- data.table::as.data.table(parcels_dmgs_sd_geo_sf)

# drop geometry column - huge because so many repeats and not needed here
parcels_dmgs_sd_geo$geometry <- NULL

# filter column
parcels_dmgs_sd_geo_dt <-parcels_dmgs_sd_geo %>% select(geoid,region_name,region_type,year,measure,value)
