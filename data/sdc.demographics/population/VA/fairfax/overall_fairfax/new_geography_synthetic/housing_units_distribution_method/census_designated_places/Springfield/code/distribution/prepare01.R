...
shapepath = "population/VA/fairfax/overall_fairfax/new_geography_synthetic/housing_units_distribution_method/census_designated_places/Springfield/data/original/shapefile/"
springfield_geoinfos <- st_read(paste0(shapepath,"Springfield.shp"))
springfield_geoinfos84 <- st_transform(springfield_geoinfos,4326)
...
#sf::sf_use_s2(FALSE)
parcels_dmgs_springfield_geo <- st_join(springfield_geoinfos84, sf::st_as_sf(parcels_dmgs_geo), join = st_intersects)
...
parcels_dmgs_springfield_geo_sf <- parcels_dmgs_springfield_geo %>%
...
parcels_dmgs_springfield_geo <- data.table::as.data.table(parcels_dmgs_springfield_geo_sf)

# drop geometry column - huge because so many repeats and not needed here
parcels_dmgs_springfield_geo$geometry <- NULL

# filter column
parcels_dmgs_springfield_geo_dt <-parcels_dmgs_springfield_geo %>% select(geoid,region_name,region_type,year,measure,value)
...
#readr::write_csv(parcels_dmgs_hsr_geo_dt, xzfile(paste0(savepath,"va059_springfield_sdad_2019_demographics.csv.xz"), compression = 9))
