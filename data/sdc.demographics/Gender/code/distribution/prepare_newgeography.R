...
fairfax_pc_dmg <- fairfax_pc_dmg %>% select(geoid,measure,value) %>% filter(measure %in% c('male','female','total_pop'))
...
readr::write_csv(fairfax_newgeo_dmg, xzfile(paste0(savepath,"va059_hsrpdsdzc_sdad_2019_gender_demographics.csv.xz"), compression = 9))
...
arl_pc_dmg <- arl_pc_dmg %>% select(geoid,measure,value) %>% filter(measure %in% c('male','female','total_pop'))
...
# save the data ----------------------------------------------------------------------------------
savepath = "Gender/data/distribution/"
readr::write_csv(arl_newgeo_dmg, xzfile(paste0(savepath,"va013_civic_sdad_2019_gender_demographics.csv.xz"), compression = 9))
