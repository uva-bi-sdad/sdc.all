...
#upload data for arlington (demographies and geometry). filter on language
arl_pc_dmg <- NULL
for (file in list.files(path)) {
  temp0 <- read_csv(xzfile(paste0(path,file)))
  temp0 <- temp0 %>% filter(str_detect(measure,'total_hh|hh_limited_english'))
...
readr::write_csv(baseline_data, xzfile(paste0(savepath,"va_civichsrsdpdzccttrbg_sdad_",min(yearlist),'_',max(yearlist),"_language_demographics_parcels.csv.xz"), compression = 9))
