...
variables <- load_variables(2017, "acs5/subject", cache = TRUE)
...
years <- c(2017:2021) # years where S0101_C03_001 is male total
...
get_acs(geography = "county", variable = "S0101_C03_001", state = "VA", year = 2017,
...
for(state in states){
  ...
  #    %>%
        # Change the measure to whatever you want, this the name that you put in the measure_info
  #      mutate(measure = "total_male", year = year, region_type = as.character(geography),
  #             measure_type = "integer", measure_units = "count") %>%
