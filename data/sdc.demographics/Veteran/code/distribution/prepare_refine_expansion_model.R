...
for (file in list.files(path)) {
  temp0 <- read_csv(xzfile(paste0(path,file)))
  temp0 <- temp0 %>% filter(str_detect(measure,'pop_veteran|vetdenom'))
...
baseline_data <- baseline_data %>% 
  mutate(measure=case_when(
    measure=="pop_veteran" ~ "veteran_count_parcels",
    measure=="perc_veteran" ~ "veteran_percent_parcels")) %>%
  filter(!is.na(value)) %>%
  mutate(geoid=as.character(geoid))
...
readr::write_csv(baseline_data, xzfile(paste0(savepath,"va_civichsrsdpdzccttrbg_sdad_",min(yearlist),'_',max(yearlist),"_veteran_demographics_parcels.csv.xz"), compression = 9))
