...
shapepath = "population/VA/fairfax/overall_fairfax/new_geography_synthetic/housing_units_distribution_method/census_designated_places/Franconia/data/original/shapefile/"
franconia_geoinfos <- st_read(paste0(shapepath,"Franconia_TSA.shp"))
franconia_geoinfos84 <- st_transform(tyson_geoinfos,4326)
...
#sf::sf_use_s2(FALSE)
parcels_dmgs_franconia_geo <- st_join(franconia_geoinfos84, sf::st_as_sf(parcels_dmgs_geo), join = st_intersects)
...
parcels_dmgs_franconia_geo_sf <- parcels_dmgs_franconia_geo %>%
...
parcels_dmgs_franconia_geo <- data.table::as.data.table(parcels_dmgs_franconia_geo_sf)

# drop geometry column - huge because so many repeats and not needed here
parcels_dmgs_franconia_geo$geometry <- NULL

# filter column
parcels_dmgs_franconia_geo_dt <-parcels_dmgs_franconia_geo %>% select(geoid,region_name,region_type,year,measure,value)

# compress and save the data
#savepath = "population/VA/fairfax/overall_fairfax/new_geography_synthetic/housing_units_distribution_method/census_designated_places/Franconia/data/distribution/"
#readr::write_csv(parcels_dmgs_hsr_geo_dt, xzfile(paste0(savepath,"va059_franconia_sdad_2019_demographics.csv.xz"), compression = 9))
